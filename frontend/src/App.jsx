...
import "./App.css";
import { getHealth, getTransactions, addTransaction } from "./api";
...
function App() {
  ...
  useEffect(() => {
    ...
    getHealth()
      .then((data) => {
        if (!cancelled) setApiStatus(data?.message || "Backend OK");
      })
      .catch(() => {
        if (!cancelled) setApiStatus("Backend not reachable");
      });
    ...
    try {
      ...
      if (newTx && typeof newTx === "object") {
        ...
        setTransactions((prev) => [
          ...prev,
          { id: Date.now(), name, amount: amountNum, type: txType },
        ]);
      }
  ...
  const handleEditTransaction = (id) => {
    ...
    const newAmount = window.prompt("Edit amount:", String(tx.amount));
  ...
  const handleGenerateInvoice = (e) => {
    e.preventDefault();
    if (!clientName.trim() || !invoiceAmount) return;
    ...
    <div className="app-root">
      <div className="card">
        <header className="card-header">
          ...
          <p style={{ marginTop: 6, opacity: 0.85, fontSize: 14 }}>
            {apiStatus}
          </p>
          ...
          <nav className="nav-buttons">
            <button
              className={`nav-chip ${
                activePage === "login" ? "nav-chip-active" : ""
              }`}
            ...
            <button
              className={`nav-chip ${
                activePage === "register" ? "nav-chip-active" : ""
              }`}
            ...
            <button
              className={`nav-chip ${
                activePage === "dashboard" ? "nav-chip-active" : ""
              }`}
            ...
            <button
              className={`nav-chip ${
                activePage === "addTransaction" ? "nav-chip-active" : ""
              }`}
            ...
            <button
              className={`nav-chip ${
                activePage === "createInvoice" ? "nav-chip-active" : ""
              }`}
    ...
    <div className="table-wrapper">
      <table className="table">
        ...
        <tbody>
          ...
          {transactions.map((tx) => (
            <tr key={tx.id ?? `${tx.name}-${tx.amount}-${tx.type}`}>
              <td>{tx.name}</td>
